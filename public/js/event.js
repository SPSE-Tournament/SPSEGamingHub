...
let countDownDate = new Date(
  document.querySelector(".event-timestamp").value
).getTime();
setInterval(function () {
  ...
  let response = `<span class='bg-red-main rounded p-2'><img src='https://img.icons8.com/ios-filled/30/000000/time.png'/>${days} days &
...
function toggleBracketFullscreen() {
  if (!document.fullscreenElement) {
    ...
    document.querySelector(
      ".fullscreenEventName"
    ).innerHTML = document.querySelector(".event-name").innerHTML;
        ...
        .then((res) => {
          ...
          res.forEach((round) => {
            ...
            round.forEach((match) => {
              ...
              divEachMatch.setAttribute(
                "class",
                "eachmatch m-3 bg-space rounded p-2 d-inline-flex flex-row justify-content-end align-items-center"
              );
              divEachRound.append(divEachMatch);
              const divNames = document.createElement("div");
              divNames.setAttribute(
                "class",
                "d-flex flex-column flex-fill p-3 mr-auto"
              );
              const divScores = document.createElement("div");
              divScores.setAttribute(
                "class",
                "d-flex align-items-center justify-content-center flex-column text-center p-3 bg-red-main rounded"
              );
              divEachMatch.append(divNames, divScores);
              const divNamesInner1 = document.createElement("div");
              divNamesInner1.setAttribute(
                "class",
                "d-flex flex-row align-items-center first-team-name text-center"
              );
              const divNamesInner2 = document.createElement("div");
              divNamesInner2.setAttribute(
                "class",
                "d-flex flex-row align-items-center second-team-name text-center"
              );
...
$(".modal-score-write").on("show.bs.modal", function (event) {
  ...
  fetch("api/match/" + button.dataset.matchid)
    ...
    .then((match) => {
      ...
      document.querySelector(".match-first-name").innerHTML =
        match.match_first_team_name;
      document.querySelector(".match-second-name").innerHTML =
        match.match_second_team_name;
      document.querySelector(".match-first-score").value =
        match.match_first_team_score;
      document.querySelector(".match-second-score").value =
        match.match_second_team_score;
      document.querySelector(".match-current-status").value =
        match.match_status;
      document.querySelector(".match-current-status").innerHTML =
        match.match_status;
    })
    .catch((err) => console.error(err));
});
