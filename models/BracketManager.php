<?php
  class BracketManager {
      ...
      public function generateMatches(array $teams, $numOfSafeRounds) {
       ...
       if ($numOfSafeRounds == 0) {
        ...
        for($i = 0; $i < count($teamsToPlay); $i+=2) {
             $matches[] = array('match_first_team' => $teamsToPlay[$i]['name'], 'match_second_team' => $teamsToPlay[$i+1]['name'], 'match_round' => 'B1');
          }
          foreach($teamsToWait as $teamName) {
             $matches[] = array('match_first_team'=>$teamName['name'], 'match_second_team' => 'freewin', 'match_round' => 'B1');
           }
        $curNumTeams = $closestExponentOf2/2;
        for($i = 0; $i < log($closestExponentOf2, 2); $i++) {
        for ($j = 0; $j < $curNumTeams; $j++) {
        $matches[] = array('match_first_team'=>'TBD', 'match_second_team' => 'TBD', 'match_round' => 'B' . ($i+2));
        }
        $curNumTeams /= 2;
        }
        return $matches;
      ...
      public function insertMatches(array $matches, $eventId) {
        try {
          foreach ($matches as $match) {
            Db::insert('matches', array('match_first_team' => $match['match_first_team'],'match_second_team' => $match['match_second_team'],'match_round' => $match['match_round'],'event_id' => $eventId));
          }
      ...
      public function returnParsedMatchesInEvent($eventId) {
        $parsedMatches = array();
        $matches = Db::multiQuery("SELECT match_id, match_first_team, match_second_team, match_round, event_id from matches
        ...
        for ($i=0; $i < $numOfRounds; $i++) {
          $roundMatches = Db::multiQuery("SELECT match_id, match_first_team, match_second_team, match_round, event_id from matches
