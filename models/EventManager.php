<?php
  class EventManager {
      public function returnEvents() {
        return Db::multiQuery("SELECT event_id, event_name, games.game_name as game_name, games.game_id as game_id, bracket_status,event_winner,
      ...
      public function returnEventByUrl($url) {
        return Db::singleQuery("SELECT event_id, event_name, games.game_name as game_name, games.game_id as game_id, bracket_status, event_winner,(select team_name from teams where team_id = event_winner)as event_winner_name,
         concat(substr(event_timestamp,9,2), '.', substr(event_timestamp,6,2), '.', substr(event_timestamp,1,4), ' ', substr(event_timestamp,12,5)) as event_parseddate,
      ...
      public function returnEventById($eventId) {
        return Db::singleQuery("SELECT event_id, event_name, games.game_name as game_name, games.game_id as game_id, bracket_status,event_winner,
         concat(substr(event_timestamp,9,2), '.', substr(event_timestamp,6,2), '.', substr(event_timestamp,1,4), ' ', substr(event_timestamp,12,5)) as event_parseddate,
         event_playerlimit, games.game_playerlimitperteam as game_plteam, event_timestamp, event_url
                                from events
                                join games on events.game_id = games.game_id
